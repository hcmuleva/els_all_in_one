 ...
 *
 * Authentication: Uses Strapi internal API (bypasses HTTP auth)
 *
 * Usage:
 * 1. Start backend: npm run dev
 * 2. In another terminal: npm run seed
 *
 * Note: This script connects directly to Strapi's database via the Node.js process,
 * so it automatically bypasses HTTP authentication. No token needed!
 */

// This script needs to be run after Strapi is started
// It connects to the running Strapi instance
const http = require("http");

const API_BASE_URL = process.env.API_URL || "http://localhost:1337";
const BEARER_TOKEN = process.env.BEARER_TOKEN || "";

// adminToken may be provided via BEARER_TOKEN env var; keep null otherwise
let adminToken =
  typeof BEARER_TOKEN !== "undefined" && BEARER_TOKEN ? BEARER_TOKEN : null; // kept for backward compatibility with older code paths
...
async function seed() {
  log.header("🌱 Starting demo data seeding...");
  log.info(`API URL: ${API_BASE_URL}\n`);

  // Use Bearer token if provided (legacy HTTP-based seed)
  if (BEARER_TOKEN) {
      ...
      try {
        ...
        if (orgResponse.status === 200 || orgResponse.status === 201) {
          ...
          for (const user of orgUsers) {
            try {
              // Strapi v5: /api/auth/local/register accepts only username, email, password
              ...
              // Prepare update payload early so we can reuse it if the user already exists
              const updatePayload = {
              ...
              if (userResponse.status === 200 || userResponse.status === 201) {
                ...
                // If duplicate (email/username taken), try to find existing user by email and update it
                ...
                if (errMsg.includes("Email or Username are already taken")) {
                  ...
                  try {
                    ...
                    const found = findResp.data?.data?.[0];
